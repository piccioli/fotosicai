...
// GET /api/verify/:token — verifica email e pubblica la foto
router.get('/:token', (req, res) => {
  ...
  if (!img) return res.status(404).send('Link non valido o foto già pubblicata.');
  ...
  db.prepare(
    "UPDATE images SET status = 'published', verified_at = ?, verification_token = NULL WHERE id = ?"
  ).run(now, img.id);
  ...
  res.redirect(302, `${PUBLIC_BASE}/?photo=${img.id}&verified=1`);
});
